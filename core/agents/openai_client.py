...
class MetricEvaluationResponse(BaseModel):
    """Structured response model for metric evaluation using OpenAI Structured Outputs."""
    score: int
    reasoning: str
    ...
    async def generate_response(
        ...
        **kwargs
        ...
        Args:
            ...
            use_structured_output: Whether to use structured outputs for metric evaluation
        ...
        try:
            ...
            # Add structured output support for metric evaluation
            if use_structured_output and BaseModel is not None:
                self.logger.debug("Using OpenAI Structured Outputs for guaranteed JSON")
                response = await self.client.beta.chat.completions.parse(
                    model=self.config.model_name,
                    messages=messages,
                    temperature=self.config.temperature,
                    response_format=MetricEvaluationResponse,
                ...
                if not response.choices or not response.choices[0].message.parsed:
                    raise LLMInvalidResponseError("Empty structured response from OpenAI API")
                
                parsed_response = response.choices[0].message.parsed
                
                # Convert structured response to JSON string for consistency
                import json
                content = json.dumps({
                    "score": parsed_response.score,
                    "reasoning": parsed_response.reasoning,
                    "improvement_advice": parsed_response.improvement_advice,
                    "positive_examples": parsed_response.positive_examples,
                    "improvement_examples": parsed_response.improvement_examples
                })
                
            else:
                # Add any additional parameters for regular calls
                request_params.update(kwargs)
                
                self.logger.debug(f"Making OpenAI API request with model: {self.config.model_name}")
                
                # Call the OpenAI Chat Completions API (regular mode)

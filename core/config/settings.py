...
def save_app_config(config: AppConfig, config_path: str = "config.toml") -> None:
    """Save application configuration to TOML file."""
    config_file = Path(config_path)
    
    # Convert AppConfig to a dictionary suitable for TOML
    data_to_save = {
        "llm": {
    ...
    # Add provider-specific API keys/base URLs if they exist
    if config.llm.provider == "openai" and config.llm.api_key:
        data_to_save["openai"] = {"api_key": config.llm.api_key}
    elif config.llm.provider == "ollama" and config.llm.base_url:
        data_to_save["ollama"] = {"base_url": config.llm.base_url}
    elif config.llm.provider == "lmstudio" and config.llm.base_url:
        data_to_save["lmstudio"] = {"base_url": config.llm.base_url}
